...
func TestRouteSyncReconcile(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := rsReconciler.Reconcile(ctrl.Request{})
...
func validateRoutes(t *testing.T, client routeclient.Interface, expectedRoutes []*routev1.Route) {
	...
	routes, err := client.RouteV1().Routes(testNamespace).List(metav1.ListOptions{
	...
	for _, expectedRoute := range expectedRoutes {
		route, err := client.RouteV1().Routes(testNamespace).Get(expectedRoute.Name, metav1.GetOptions{})
